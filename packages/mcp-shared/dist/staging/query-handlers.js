...
async function listStagedDatasets(doNamespace, toolPrefix, scope) {
    ...
    try {
        ...
        if (datasets.length === 0) {
            return createCodeModeResponse({ staged_datasets: [], message: "No staged datasets found for this session." }, { textSummary: "No staged datasets found for this session." });
        }
        ...
        return createCodeModeResponse({ staged_datasets: listing, hint: "Call this tool with a specific data_access_id to get the full schema for that dataset." }, { textSummary: `Found ${listing.length} staged dataset(s) in this session.` });
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
