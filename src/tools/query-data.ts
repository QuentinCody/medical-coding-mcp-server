...
interface QueryEnv {
    MEDICAL_CODING_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("MEDICAL_CODING_DATA_DO", "coding");

    server.registerTool(
        "coding_query_data",
        {
            title: "Query Staged Medical Coding (HCPCS) Data",
            description:
                "Query staged Medical Coding (HCPCS) data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
